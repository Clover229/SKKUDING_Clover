...
const PoketmonList = new Map<string, PoketmonInfo>();
...
//느낌표의 의미: content는 null이 반드시 아니다
//querySelector 대신 getbyElementbyClassName을 썼더니 안됐음. 와이/
PoketmonList.forEach((pokemon, name) => {
    ...
    card.addEventListener('click', ()=> {
      ...
      window.localStorage.setItem(
          "pokemon",
          JSON.stringify({ id: currentIndex, name:name, ...pokemon })
        );
          
    });
    const imgcard = document.createElement("img");
    imgcard.src = `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/official-artwork/${
        index + 1
      }.png`;
    ...
    //innertext와 textContext 차이? textContext는 스타일까지 가져온다
